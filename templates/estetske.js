// Auto-extracted from n8n workflow OckyZOlduxFDVzj0 (Opsis - Email Sequence v5 - Estetske poliklinike)
// Industry: estetske
module.exports = {
  E1: {
    V1: {
...
vidim da {{website}} trenutno vraca gresku ({{website_status}}). Posjetitelji koji Vas Googlaju trenutno samo dobivaju prazan ekran.
...
slucajno sam naletio na Vas Google Business profil dok sam trazio {{Kategorija_lc}} u {{Grad}}u - {{review_count}} recenzija, ocjena {{review_rating}}, sve stvari koje ulijevaju povjerenje.

Primijetio sam jednu malu stvar koja Vam moze povecati broj poziva za 15-20% bez ikakvog troska:

Google profil ima sekciju "Postovi" (slicno kao Facebook postovi) koju vecina ljudi uopce ne koristi. Google penaliziraj profile bez recentnih postova - ako nista ne objavljujete tjednima, profil pada u rangu za lokalne pretrage.

Sto treba: 1 post tjedno, ne mora biti dugacak. Primjeri:
- Nova ponuda ovaj tjedan (foto + 2 recenice)
- Sezonska tema ("ljetni tretmani", "uskrsnje radno vrijeme")
- Recenzija klijenta + Vas odgovor
...
otvorio sam {{website}} u Chromeu i odmah dobio upozorenje "Veza nije sigurna". Razlog: stranica nema HTTPS (SSL certifikat).
...
Sitemap je tekstualni fajl koji govori Googleu "evo svih stranica koje imam, indeksiraj ih". Bez njega Google sam pogada sto imate, i obicno propusti pola stranica.

Generirao sam okvirni sitemap za Vasu polikliniku, kopirajte u obican tekst editor, snimite kao "sitemap.xml":

  <?xml version="1.0" encoding="UTF-8"?>
  <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    <url><loc>{{website}}/</loc><priority>1.0</priority></url>
    <url><loc>{{website}}/o-nama</loc><priority>0.8</priority></url>
...
Ako imate vise stranica od ovih u primjeru, javite ih, pa Vam prilagodim sitemap.
...
- Ne znate odakle dolaze (Google, Instagram, direktno...)
- Ako pokrenete oglase na Facebooku ili Googleu, ne mozete optimizirati jer alati ne vide tko je konvertirao
...
Za 24-48h imat cete prve podatke o tome tko dolazi na stranicu, koja je najgledanija, koliko se zadrzavaju.
...
opsisdalmatia.com`
    ...
    V6: {
      subject: `gotovi WhatsApp odgovori za Vasu kliniku`,
      body: `Pozdrav,

pretpostavljam da preko WhatsAppa i Instagrama dobivate puno pitanja tipa "Imate li termin u srijedu?", "Koliko kosta botox?", "Trebam li nesto prije tretmana?". Vjerojatno osoblje na recepciji rucno tipka iste odgovore svaki dan.
...
1) Pozdrav (kracenica: /pozdrav)
"Pozdrav i hvala sto ste nas kontaktirali. Radno vrijeme: Pon-Pet 9-18, Sub 9-13. Za rezervaciju nam posaljite ime, tretman i 2-3 termina koja Vam odgovaraju - javimo se u 1h."

2) Cjenik (kracenica: /cjenik)
"Nas cjenik za najtrazenije tretmane: hijaluronski filer 280-500?, botox 250-450?, tretmani lica od 80?. Svi tretmani s licenciranom lijecnicom. Za personalizirani prijedlog napisite sto Vas zanima."

3) Prije tretmana (kracenica: /prije)
"Prije botox tretmana: bez aspirina/ibuprofena 7 dana, bez alkohola 24h. Nakon tretmana: 2-3 dana blago crvenilo. Rezultati traju 4-6 mj."

4) Recall (mozete poslati rucno 3 mj nakon tretmana)
"Pozdrav! Vas zadnji tretman bio je prije 3 mjeseca - vrijeme je za osvjezavanje rezultata. Javite kada Vam odgovara, rezerviram termin."

Postavlja se za 5 minuta: WhatsApp Business ? Settings ? Business tools ? Quick replies ? Add new.

Rijesit ce Vam otprilike 70% rutinskih pitanja, recepcija ce se moci posvetiti slozenijim upitima.
...
testirao sam {{website}} kroz Google PageSpeed Insights - mobile rezultat je {{pagespeed_mobile}}/100. Sve ispod 50 znaci da posjetitelji cekaju 4-6 sekundi da se stranica ucita, a 53% ljudi odustane prije nego sto se uopce prikaze (Google podatak).
...
primijetio sam jednu malu stvar na {{website}} koja utjece na to kako Vasa stranica izgleda kad se pojavi u Google rezultatima ili kad netko podijeli link na Facebooku/WhatsAppu.
...
- Kad netko podijeli Vas link, prikaze se prazno (bez slike, bez opisa)
...
Napisao sam Vam gotove tagove, samo paste u <head> sekciju HTML-a:
  ...
  <meta name="description" content="{{title}} - {{Kategorija_lc}} u {{Grad}}u. {{review_count}}+ recenzija, ocjena {{review_rating}}/5. Rezervirajte online ili nazovite {{phone}}.">

  <!-- Open Graph (za Facebook/WhatsApp/LinkedIn share) -->
  <meta property="og:title" content="{{title}} - {{Kategorija_lc}} {{Grad}}">
  <meta property="og:description" content="{{Kategorija_lc}} u {{Grad}}u, {{review_count}}+ zadovoljnih klijenata.">
...
Paste sve u <head> nakon <title> taga. Za "cover.jpg" upload neku sliku Vase klinike u root domena (ili promijenite path u onaj koji imate). Velicina 1200x630px ide najbolje.
...
Posto je tehnicka strana poslozena, evo 5 sitnica koje obicno pomazu estetskim klinikama dobiti vise rezervacija iz iste kolicine prometa:
...
2) WhatsApp click button
   Dodajte negdje vidljivo na stranicu:
   <a href="https://wa.me/{{phone_intl}}?text=Pozdrav,%20zanima%20me%20termin">Posaljite poruku na WhatsApp</a>
   Klijent klikne, otvara WhatsApp s predispunjenom porukom.

3) Sticky "Rezerviraj termin" gumb
   Fiksni gumb dolje desno na svakoj stranici, vodi na kontakt formu ili Calendly. Conversion rate +25-40% testirano (Hotjar 2024 data).

4) Trust elementi iznad fold-a (prvih 600px stranice)
   "{{review_count}}+ klijenata", "{{review_rating}}/5 Google", broj godina iskustva. Smanjuje bounce za ~15%.

5) Mjesecni speed re-test

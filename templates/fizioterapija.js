// Auto-extracted from n8n workflow jQHmdf189zHHulVl (Opsis - Email Sequence v5 - Fizioterapeutske ordinacije)
// Industry: fizioterapija
module.exports = {
  E1: {
    V1: {
...
vidim da {{website}} trenutno vraca gresku ({{website_status}}). Pacijenti koji Vas Googlaju ("fizioterapeut {{Grad}}", "rehabilitacija {{Grad}}") trenutno samo dobivaju prazan ekran - i ne mogu pronaci radno vrijeme, planove terapija ni kontakt, sto je problem kad netko hitno trazi termin nakon operacije ili ozljede.
...
slucajno sam naletio na Vas Google Business profil dok sam trazio fizioterapeuta u {{Grad}}u - {{review_count}} recenzija, ocjena {{review_rating}}, sve stvari koje pacijentima ulijevaju povjerenje kad biraju gdje ce ici na rehabilitaciju.

Primijetio sam jednu malu stvar koja Vam moze smanjiti broj telefonskih poziva s pitanjima o cijenama, paketima i HZZO uputnicama za 30-40% bez ikakvog troska:

Google profil ima sekciju "Postovi" (slicno kao Facebook postovi) koju vecina ordinacija uopce ne koristi. Google penalizira profile bez recentnih postova - ako nista ne objavljujete tjednima, profil pada u rangu za lokalne pretrage tipa "fizioterapija {{Grad}}" ili "rehabilitacija nakon operacije {{Grad}}".

Sto treba: 1 post tjedno ili dva tjedno, ne mora biti dugacak. Primjeri za fizioterapeutsku ordinaciju:

- Informacija o uslugama ("manualna terapija - kada pomaze", "shock wave za tenisaca", "kineziotape za sportase")
- Obavijest o paketima i HZZO uputnicama ("primamo dopunsko osiguranje Croatia/Allianz", "individualne terapije")
- Edukativni post ("3 vjezbe za donji dio leda", "kako se pripremiti za prvi termin nakon operacije koljena")

Postavlja se za 2 minute preko Google Business aplikacije na mobitelu. Sigurno cete primijetiti razliku za 2-3 tjedna - manje rutinskih poziva u recepciju, a pacijenti koji prvi put traze terapiju brze Vas pronadu.
...
otvorio sam {{website}} u Chromeu i odmah dobio upozorenje "Veza nije sigurna". Razlog: stranica nema HTTPS (SSL certifikat). Za fizioterapeutsku ordinaciju to je bitno - pacijenti koji pretrazuju terapije i citaju o planovima rehabilitacije obicno odmah zatvore tab kad vide "Veza nije sigurna" jer pretpostavljaju da je stranica zanemarena ili nije ozbiljna.
...
Sitemap je tekstualni fajl koji govori Googleu "evo svih stranica koje imam, indeksiraj ih". Bez njega Google sam pogada sto imate, i obicno propusti pola stranica - sto je steta jer bas /terapije i /sportska stranice najvise pomazu pacijentima da odluce prije prvog dolaska na rehabilitaciju.

Generirao sam okvirni sitemap za fizioterapeutsku ordinaciju, kopirajte u obican tekst editor, snimite kao "sitemap.xml":

  <?xml version="1.0" encoding="UTF-8"?>
  <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    <url><loc>{{website}}/</loc><priority>1.0</priority></url>
    <url><loc>{{website}}/terapije</loc><priority>0.9</priority></url>
    <url><loc>{{website}}/sportska</loc><priority>0.9</priority></url>
...
Ako imate dodatne stranice (npr. manualna terapija, shock wave, kineziotape, post-op rehabilitacija, terapija u vodi), javite ih, pa Vam prilagodim sitemap.
...
- Ne znate koje informacije najvise traze (cjenik, paketi, HZZO uputnice, sportska rehabilitacija)
...
Za 24-48h imat cete prve podatke o tome tko dolazi i koja je najgledanija stranica. Npr. ako vidite da 60% klikova ide na "sportska rehabilitacija", znate da Vam tu uslugu treba istaknuti na home - a marketing prema sportskim klubovima ima smisla.

Napomena: GA4 ne prati osobne zdravstvene podatke, samo agregirani saobracaj - GDPR-kompatibilno uz standardni cookie banner. Ne prikupljaju se podaci o pacijentima ni o tipu ozljede.
...
Ne zelite vise dobivati ove mailove? Odgovorite "STOP HRVATSKA".`
    ...
    V6: {
      subject: `WhatsApp odgovori za terapijske upite`,
      body: `Pozdrav,

pretpostavljam da preko WhatsAppa i telefona dobivate puno ponavljajucih pitanja - "Koliko kosta paket?", "Primate li HZZO uputnice?", "Imate li termin za koljeno?", "Treba li nalaz prije prvog dolaska?". Vecina pacijenata se javi s istih 4-5 pitanja, a recepcija rucno tipka odgovore svaki dan.

WhatsApp Business aplikacija ima sekciju "Quick Replies" koja vecini to rijesi - pacijent pise umjesto zove, recepcija odgovori u pauzi izmedu terapija. Evo 4 gotova odgovora koje mozete kopirati i koristiti:

1) Prvi upit i triage (kracenica: /upit)
"Pozdrav. Da Vam sto brze javimo plan terapije: posaljite nam vrstu tegobe (akutna ozljeda, kronicna bol, post-operativna rehabilitacija), koliko dugo traje, jeste li bili kod lijecnika i imate li nalaze (RTG, MR, otpusno pismo). Ako je sportska ozljeda, dodajte koji sport i u kojoj fazi je natjecanje."

2) Plan terapije i paketi (kracenica: /plan-terapije)
"Standardni paket 10 sesija - popust u odnosu na pojedinacnu cijenu. Individualna sesija dostupna kad nema potrebe za punim ciklusom. Kombiniramo manualnu terapiju, strojeve (elektroterapija, ultrazvuk, shock wave) i terapiju u vodi (gdje je indicirano). Tocan plan dobit cete nakon prvog pregleda."

3) HZZO i osiguranje (kracenica: /osiguranje)
"HZZO uputnice prihvacamo. Dopunska osiguranja: Croatia, Allianz, Wiener - sve podrzavamo, javite koje imate. Gotovinske terapije takoder, izdajemo R1 racun ako Vam treba refundacija. Za prvu informaciju o pokrivenosti posaljite tip uputnice i broj sesija koji Vam je odobren."
...
testirao sam {{website}} kroz Google PageSpeed Insights - mobile rezultat je {{pagespeed_mobile}}/100. Sve ispod 50 znaci da pacijenti cekaju 4-6 sekundi da se stranica ucita, a 53% ljudi odustane prije nego sto se uopce prikaze (Google podatak). Vecina pacijenata Vas trazi s mobitela ("fizioterapija {{Grad}}", "rehabilitacija nakon operacije {{Grad}}") - ako se ne otvori brzo, nazovu sljedecu na popisu.

3 stvari koje obicno najvise ubrzaju (lako za napraviti):

1) Komprimirajte slike
   Idite na tinypng.com, uploadajte sve slike s Vase stranice (fotografije tima, sale, strojeva), downloadajte komprimirane verzije, zamijenite original. Obicno smanji slike za 60-70% bez vidljivog gubitka kvalitete.
...
3) Aktivirajte browser caching
   ...
   Tako returning pacijenti (oni koji se vracaju provjeriti raspored terapija ili paket cijena) ne moraju ponovno ucitavati slike.
...
primijetio sam jednu malu stvar na {{website}} koja utjece na to kako Vasa stranica izgleda kad se pojavi u Google rezultatima ili kad netko podijeli link na Facebooku/WhatsAppu (npr. pacijent koji preporucuje Vasu rehabilitaciju nekom kolegi nakon operacije).
...
Napisao sam Vam gotove tagove, samo paste u <head> sekciju HTML-a:
  ...
  <meta name="description" content="{{title}} - fizioterapija u {{Grad}}u. {{review_count}}+ recenzija, ocjena {{review_rating}}/5. HZZO uputnice, sportska rehabilitacija. Zakazivanje na {{phone}}.">

  <!-- Open Graph (za Facebook/WhatsApp/LinkedIn share) -->
  <meta property="og:title" content="{{title}} - fizioterapija {{Grad}}">
  <meta property="og:description" content="Fizioterapeutska ordinacija u {{Grad}}u, {{review_count}}+ zadovoljnih pacijenata. Rehabilitacija, sportske ozljede, post-op.">
...
Paste sve u <head> nakon <title> taga. Za "cover.jpg" upload neku sliku sale ili tima u root domena (ili promijenite path u onaj koji imate). Velicina 1200x630px ide najbolje - sala s opremom ili tim u radu daju najbolji dojam.
...
Posto je tehnicka strana poslozena, evo 5 sitnica koje obicno pomazu fizioterapeutskim ordinacijama dobiti vise organiziranih termina i smanjiti no-show iz iste kolicine prometa:

1) Telefonski broj klikabilan na mobitelu
   U HTML-u broj treba biti zamotan u <a href="tel:{{phone_clean}}">{{phone}}</a> - kad pacijent s mobitela klikne, otvara dial direktno. Tipicno 10-20% vise poziva ako trenutno nije klikabilan.

2) WhatsApp click button za zakazivanje
   Dodajte negdje vidljivo na stranicu:
   <a href="https://wa.me/{{phone_intl}}?text=Pozdrav,%20trebam%20termin%20za%20fizioterapiju">Zakazi termin na WhatsAppu</a>
   Pacijent klikne, otvara WhatsApp s predispunjenom porukom. Smanjuje opterecenje telefona dok ste s pacijentom na terapiji i daje recepciji vrijeme za organizaciju.

3) Sticky "Zakazi termin" gumb
   Fiksni gumb dolje desno na svakoj stranici, vodi na kontakt formu ili WhatsApp. Conversion rate +25-40% testirano (Hotjar 2024 data). Posebno korisno za pacijente koji citaju o specificnoj terapiji (npr. shock wave) i odmah zele rezervirati.

4) Trust elementi iznad fold-a (prvih 600px stranice)
   "{{review_count}}+ recenzija", "{{review_rating}}/5 Google", "HZZO i dopunska osiguranja", godine iskustva, broj fizioterapeuta u timu. Za sportsku rehabilitaciju posebno dobro radi referenca na klubove ili sportase s kojima ste radili. Bounce pada za ~15%.

5) SMS/WhatsApp podsjetnik 24h prije termina
   No-show rate u HR fizioterapeutskim ordinacijama je tipicno 12-18% (pacijent se osjeca bolje pa preskoci terapiju - sto rusi plan rehabilitacije). Automatski podsjetnik dan ranije ("Podsjetnik: sutra u 10:00 imate terapiju kod {{title}}. Ponesite rucnik i nalaze. Ako ne stizete, odgovorite OTKAZ.") spusta to na 4-6%. Postavlja se kroz WhatsApp Business ili SMS gateway.
